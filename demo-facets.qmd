...
1. On the Name column, use the dropdown to Edit Column > Add a column based on this column. Like this:
...
1. In our case here, just **click OK to create the new column**.
...
1. Click on the dropdown for the Candidate column and go to Edit cells > Common transforms > **Trim leading and trailing whitespace**.
...
1. Use the dropdown on the "Party" column to choose Edit cells > Common transforms > **To uppercase**.
...
1. Go to the dropdown for Candidate and then to Facet > **Text facet**.
...
1. Find "Citizens For Joe Crabb (DISSOLVED)" and choose **edit**.
2. Change the text to say just "Joe Crabb" and hit **Apply**.

Once you change that value, the list sorts again and Joe is down by the J's.

### Practice editing from a text facet

A little work on your own to get some practice:

1. Close the Candidate text facet by **clicking the `X`** that is to the left of the facet title.
1. Clean up the Race-Ethnicity values to spell out those values to Asian, Black, Hispanic, White. This is straightforward facet editing: Create the facet, then edit the values.
...
1. From the dropdown on Candidate, choose **Text filter**.
...
Now we'll use the Transform function to search for and replace values.

1. Set your Text filter to "Friends of" so we can see them, but don't fix them. We'll instead edit all the rows at once using Transform. Click on the Candidate dropdown and go to Edit cells > **Transform**:
...
1. Use the Year column dropdown to choose **Edit Column > Add column based on this column**.
1. Name the column **"Election cycle"**.
1. In the expression field, just add two quotes with nothing between them: `""`.
1. Click **OK**.
...
1. In the text filter, check the box next to **regular expression**.
...
1. Use the dropdown on **Election cycle** to choose Edit cells > Transform.
...
1. Go under the Export button at top-right and choose the export type of your choice.
